# Pong V3
# This is the final version of the program. This is a game where a ball bounces 
# off the 4 edges of the screen and paddles. If it hits the left or right edge 
# of the screen, the opposite side gets a point. If it hits the inner side of 
# the paddle, it bounces; otherwise, it goes through. This version also allows 
# paddle movements.
...
class Game:
    def __init__(self, window):
        ...
        self.score = [0,0]    
        self.rect_heights = [170, 170]
        
        self.rect_left = Rect((90,self.rect_heights[0]),(10,60))
        self.rect_right = Rect((400,self.rect_heights[1]),(10,60))    
        pygame.key.set_repeat(20, 20)
        self.balls = [ball(6, "white", [9, 3], self.window, [250, 200], self.score, self.rect_left, self.rect_right)] # ball properties in an array, can add more ball objects
        self.pause_time = 1/90  #how much time to wait between updates, smaller time is faster game, but also choppier
    ...
    def handle_events(self):
        ...
        if self.game_over == False:  
            ...
            self.paddle_movement(list_of_keys)         
    ...
    def paddle_movement(self, list_of_keys):
        self.list_of_keys = list_of_keys
        if list_of_keys[K_a] == True:            
            if ((self.balls[0].rect_left.top > 0) and (self.balls[0].rect_left.top < (self.window.get_height() - 60))):
                self.balls[0].rect_left.top += 5
            elif (self.balls[0].rect_left.top == 0):
                self.balls[0].rect_left.top += 5
                
        if list_of_keys[K_q] == True:
            if ((self.balls[0].rect_left.top > 0) and (self.balls[0].rect_left.top < (self.window.get_height() - 60))):
                self.balls[0].rect_left.top -= 5
            elif (self.balls[0].rect_left.top == self.window.get_height() - 60):
                self.balls[0].rect_left.top -= 5
                
        if list_of_keys[K_l] == True:
            if ((self.balls[0].rect_right.top > 0) and (self.balls[0].rect_right.top < (self.window.get_height() - 60))):
                self.balls[0].rect_right.top += 5
            elif (self.balls[0].rect_right.top == 0):
                self.balls[0].rect_right.top += 5
        if list_of_keys[K_p] == True:
            if ((self.balls[0].rect_right.top > 0) and (self.balls[0].rect_right.top < (self.window.get_height() - 60))):
                self.balls[0].rect_right.top -= 5
            elif (self.balls[0].rect_right.top == self.window.get_height() - 60):
                self.balls[0].rect_right.top -= 5           
...
class ball:
    ...
    def screen_bounce(self):
        ...
        if self.position[0] <= 0:
            ...
            self.score_change('left')
        elif self.position[0] >= self.window.get_width():
            self.velocity[0] = -self.velocity[0]  
            self.score_change('right')
